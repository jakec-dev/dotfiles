...
function check_updates() {
    while true; do
        ...
        if (( $updates > 15 )); then
            dunstify -u critical "UPDATE YOUR FUCKING PACKAGES!" "$updates packages are ready to be updated"
        elif (( $updates > 0 )); then
...
case "$1" in
    update)
        bspc rule -a \* state=floating && kitty bash -c "yay -Syu --answerclean yes --nodiffmenu --removemake"
        ;;
    *) 
        check_updates 
        ;;
